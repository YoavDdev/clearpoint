-- =====================================================
-- Clearpoint Security - Payment & Subscription Schema
-- =====================================================

-- ניקוי מלא - מחיקת כל מה שקשור לתשלומים
...
-- הסרת עמודות מטבלת users (אם קיימות)
ALTER TABLE users DROP COLUMN IF EXISTS subscription_id CASCADE;
ALTER TABLE users DROP COLUMN IF EXISTS payment_method_on_file CASCADE;
ALTER TABLE users DROP COLUMN IF EXISTS billing_email CASCADE;
ALTER TABLE users DROP COLUMN IF EXISTS tax_id CASCADE;
...
CREATE TABLE subscriptions (
  ...
  CONSTRAINT fk_plan_subscription FOREIGN KEY (plan_id) REFERENCES plans(id)
);
...
-- טבלת היסטוריית מנויים (לעקוב אחרי שינויים)
CREATE TABLE subscription_history (
  ...
  CONSTRAINT fk_subscription_history FOREIGN KEY (subscription_id) REFERENCES subscriptions(id) ON DELETE CASCADE
);
...
-- עדכון טבלת users - הוספת שדות רלוונטיים לתשלומים
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS tax_id TEXT; -- ח.פ / ת.ז לחשבונית
...
-- תצוגה של מנויים פעילים עם פרטי משתמש
...
-- תצוגה של מנויים שצריך לחייב בקרוב
