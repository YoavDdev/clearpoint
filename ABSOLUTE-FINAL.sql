-- =====================================================
-- ABSOLUTE FINAL - הפתרון הסופי ביותר
-- =====================================================
...
CREATE POLICY "Admins can view all subscriptions" ON subscriptions FOR ALL USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'));
CREATE POLICY "Users can view their own subscription history" ON subscription_history FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Admins can view all subscription history" ON subscription_history FOR ALL USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'));
...
WHERE s.status = 'active' AND s.next_billing_date IS NOT NULL AND s.next_billing_date <= CURRENT_DATE + INTERVAL '7 days'
ORDER BY s.next_billing_date ASC;

COMMENT ON TABLE payments IS 'כל התשלומים במערכת';
COMMENT ON TABLE subscriptions IS 'מנויים פעילים';
COMMENT ON TABLE subscription_history IS 'היסטוריה של שינויים';

SELECT '🎉 SUCCESS! Payment system created! 🎉' as status;
